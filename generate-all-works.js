...
/**
 * Generate ALL 80 missing Indian works from Project Gutenberg
 * Processes HIGH, MEDIUM, and LOW priority works
 */
...
// Load complete works database
const database = JSON.parse(fs.readFileSync('./complete-works-database.json', 'utf8'));

// Description generators by work type
...
function generateDescription(work) {
  ...
  return {
    full: `"${work.title}" by ${work.author}, published in ${work.year}, represents an important work related to Indian literary and cultural heritage. This text contributes to understanding India's history, society, or cultural traditions during a significant period of transformation.`
  };
}

async function processAllWorks() {
  console.log('╔═══════════════════════════════════════════════════════════════════════════╗');
  console.log('║   GENERATING ALL 80 MISSING GUTENBERG WORKS FOR DHWANI                   ║');
  console.log('╚═══════════════════════════════════════════════════════════════════════════╝\n');

  const results = { HIGH: [], MEDIUM: [], LOW: [], skipped: [] };
  let totalGenerated = 0;

  // Process each priority level
  for (const priority of ['HIGH_PRIORITY', 'MEDIUM_PRIORITY', 'LOW_PRIORITY']) {
    const works = database[priority] || [];
    const displayPriority = priority.replace('_PRIORITY', '');

    console.log(`\n${'🔴🟡⚪'[['HIGH_PRIORITY', 'MEDIUM_PRIORITY', 'LOW_PRIORITY'].indexOf(priority)]} PROCESSING ${displayPriority} PRIORITY (${works.length} works)...\n`);

    for (const work of works) {
      // Skip already completed works
      if (work.status === 'COMPLETED') {
        results.skipped.push(work.title);
        continue;
      }

      const customDescription = generateDescription(work);
      const workData = {
      ...
      try {
        ...
        results[displayPriority].push({ work: work.title, ...result });
        totalGenerated++;

        // Progress indicator every 10 works
        if (totalGenerated % 10 === 0) {
          console.log(`   [Progress: ${totalGenerated} works generated]\n`);
        }
      } catch (error) {
        console.error(`❌ Error processing ${work.title}:`, error.message);
        results[displayPriority].push({ work: work.title, error: error.message });
      }
    }
  }
  ...
  // Final Summary
  console.log('\n' + '═'.repeat(77));
  console.log('FINAL SUMMARY - ALL WORKS GENERATION');
  console.log('═'.repeat(77));

  const highSuccess = results.HIGH.filter(r => !r.error).length;
  const mediumSuccess = results.MEDIUM.filter(r => !r.error).length;
  const lowSuccess = results.LOW.filter(r => !r.error).length;
  const totalSuccess = highSuccess + mediumSuccess + lowSuccess;

  console.log(`\n✅ HIGH Priority:   ${highSuccess} works generated`);
  console.log(`✅ MEDIUM Priority: ${mediumSuccess} works generated`);
  console.log(`✅ LOW Priority:    ${lowSuccess} works generated`);
  console.log(`⏭️  Skipped (already completed): ${results.skipped.length} works`);
  console.log(`\n🎉 TOTAL GENERATED: ${totalSuccess} works`);
  console.log(`📁 All files in: new-gutenberg-works-2025/\n`);

  return results;
}

// Run
processAllWorks().then(results => {
  console.log('✅ Generation complete! All works ready for review.\n');
}).catch(console.error);

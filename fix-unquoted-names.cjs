...
recentFiles.forEach(fileName => {
  ...
  // Pattern: name: value (not starting with quote) that contains a colon
  const namePattern = /^(\s+name): ([^"'][^:\n]*):([^\n]*)$/gm;

  content = content.replace(namePattern, (match, indent, part1, part2) => {
    const value = (part1 + ':' + part2).trim();
    fixes++;
    return `${indent}: "${value}"`;
  });
